...
process TRAIN_FINAL {
    ...
    input:
    tuple val(meta), path(csv_file), path(best_hpo_params)

    output:
    tuple val(meta), path("final_model_weights"), emit: model
    tuple val(meta), path("final_model_weights/wandb_run_id.txt"), emit: wandb_run_id
    ...
    train.py \\
        ...
        --output_dir ./final_model_weights \\
